...
export async function GET(request: NextRequest) {
  try {
    ...
    const fid = searchParams.get('fid') || '874349' // Default to 874349 if not provided
    ...
    const objectKey = `regen-energy-chart/${filename}`

    // Get the base URL from the current request
    const requestUrl = new URL(request.url)
    // Force http for localhost, keep https for production
    ...
    if (profilePicture)
      ogImageUrl.searchParams.set('profilePicture', profilePicture)
    ...
    if (!ogImageResponse.ok) {
      ...
      throw new Error(
        `Failed to generate OG image: ${ogImageResponse.status} ${ogImageResponse.statusText}. Details: ${errorText}`,
      )
    }
